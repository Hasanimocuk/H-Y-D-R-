...
exports.run = async(client, message, args) => {
  ...
  if (!args[0]) return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`s.link aç\` | Kapatmak İstiyorsanız \`s.link kapat\` Yazabilirsiniz`)
  if (args[0] !== 'aç' && args[0] !== 'kapat') return message.channel.send(`:no_entry: Reklam Filtresini Ayarlamak İçin \`s.link aç\` | Kapatmak İstiyorsanız \`s.link kapat\` Yazabilirsiniz`)

    if (args[0] == 'aç') {
    db.set(`reklamFiltre_${message.guild.id}`, 'acik')
    let i = await db.fetch(`reklamFiltre_${message.guild.id}`)
  message.channel.send(`<a:onays:773266899391741952> Reklam Filtresi başarıyla ayarlandı.`)    
